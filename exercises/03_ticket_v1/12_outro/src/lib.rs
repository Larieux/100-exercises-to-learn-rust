...
impl Order {
	...
	fn check_product_name(product_name: &String) {
		...
		assert!(product_name.len() < 300, "product name cannot be more
		than 300 bytes long")
	}

	fn check_quantity(quantity: &u32) {
		assert!(quantity > &0u32, "quantity must be greater than zero")
	}

	fn check_unit_price(unit_price: &u32) {
		assert!(unit_price > &0u32, "unit_price must be greater than zero")
	}
